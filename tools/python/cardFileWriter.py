...
from Stop_Run2.tools.helpers import writeObjToFile, natural_sort
...
#        limit["{0:.3f}".format(round(qE.GetValue(),3))] = preFac*l.GetValue()
    ...
    def calcNuisances(self, fname=None, options="",bonly=False, numToysForShape=200):
        ...
        print "Creating %s"%uniqueDirname
        os.makedirs(uniqueDirname)
        shutil.copyfile(os.path.join(os.environ['CMSSW_BASE'], 'src', 'Stop_Run2', 'tools', 'python', 'diffNuisances.py'), os.path.join(uniqueDirname, 'diffNuisances.py'))
